...
int main(int nargs, char** argv){
  ...
  // loop through all the entries in the file
    ...
    larcv::EventImage2D* ev_in_adc_dlreco_thrumu  = (larcv::EventImage2D*)(io_larcv->get_data(larcv::kProductImage2D,"thrumu"));
    larcv::EventImage2D* ev_in_adc_dlreco_wire  = (larcv::EventImage2D*)(io_larcv->get_data(larcv::kProductImage2D,"wire"));
    ...
    int run_thrumu = ev_in_adc_dlreco_thrumu->run();
        ...
        std::cout << "thrumu identifying info:\n";
		std::cout << run_thrumu << " " << subrun_thrumu << " " << event_thrumu << "\n";
    ...
    int run_wire = ev_in_adc_dlreco_wire->run();
        ...
        std::cout << "wire identifying info:\n";
        std::cout << run_wire << " " << subrun_wire << " " << event_wire << "\n";
    ...
    // column = time tic, row = wire number
	  for (int col = 0; col<3456;col++){
		...
		double thrumu_val_u = img_2d_in_v_thrumu[0].pixel(row,col);
		double thrumu_val_v = img_2d_in_v_thrumu[1].pixel(row,col);
		double thrumu_val_y = img_2d_in_v_thrumu[2].pixel(row,col);
        
        //sets pixel value to 0 if thrumu says it is background
        //otherwise, gets the pixel value from wire
        //u plane
        if (thrumu_val_u != 0){
            val_u = 0;
        } else {
            val_u = img_2d_in_v_wire[0].pixel(row,col);
        }
        //v nplanes
        if (thrumu_val_v != 0){
            val_v = 0;
        } else {
            val_v = img_2d_in_v_wire[1].pixel(row,col);
        }
        //y plane
        if (thrumu_val_y != 0){
            val_y = 0;
        } else {
            val_y = img_2d_in_v_wire[2].pixel(row,col);
        }
    ...
    for (auto pgraph : test_pgraph_v) {
      for (larcv::ROI reco_roi : pgraph.ParticleArray()){
        ...
        // now project into 2d
        std::vector<int> vtx_rc = getProjectedPixel(reco_vertex, img_2d_in_v_wire[0].meta(), 3);
        // fill root histograms
    ...
    // first draw charge image
	  ev_disp_raw_u.SetTitle(Form("Image Raw No Background U Plane Run: %d Subrun: %d Event: %d",run_wire,subrun_wire,event_wire));
    ...
    vtx_u.Draw("SAME");
		can.SaveAs(Form("%s/output_%d_%d_%d_u%s.png",output_dir.c_str(),run_wire,subrun_wire,event_wire,output_custom.c_str()));

    ev_disp_raw_v.SetTitle(Form("Image Raw No Background V Plane Run: %d Subrun: %d Event: %d",run_wire,subrun_wire,event_wire));
    ...
    vtx_u.SetMarkerSize(5);
    vtx_v.Draw("SAME");
		can.SaveAs(Form("%s/output_%d_%d_%d_v%s.png",output_dir.c_str(),run_wire,subrun_wire,event_wire,output_custom.c_str()));

    ev_disp_raw_y.SetTitle(Form("Image Raw No Background Y Plane Run: %d Subrun: %d Event: %d",run_wire,subrun_wire,event_wire));
    ...
    vtx_u.SetMarkerSize(5);
    vtx_y.Draw("SAME");
    can.SaveAs(Form("%s/output_%d_%d_%d_y%s.png",output_dir.c_str(),run_wire,subrun_wire,event_wire,output_custom.c_str()));
