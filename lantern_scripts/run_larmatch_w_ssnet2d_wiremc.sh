...
#CMD="python3 $SSNET_DIR/inference_sparse_ssnet_uboone.py -i $larsoftout -w $SSNET_DIR/weights -o ssnet_output.root --adcname ${adcname}"
CMD="python3 ./inference_sparse_ssnet_uboone.py -i $larsoftout -w $SSNET_DIR/weights -o ssnet_output.root --adcname ${adcname}"
echo $CMD
$CMD
...
python3 ./recreate_ubspurn.py -i ${baseinput} -o ssnet_ubspurn_output.root --adcname ${adcname}
inputTrees=$(rootls ssnet_ubspurn_output.root)
for tree in $inputTrees; do
  rootcp ssnet_ubspurn_output.root:${tree} ${baseinput}
done

CMD="python3 $LARMATCH_DIR/deploy_larmatchme.py --config-file ${CONFIG_FILE} --supera $baseinput --weights ${LARMATCH_DIR}/${WEIGHT_FILE} --output $lm_outfile --min-score 0.5 --adc-name wire --chstatus-name ${adcname} --device-name cpu --use-skip-limit"
echo $CMD
$CMD
